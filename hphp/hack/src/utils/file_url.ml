...
let dos_url_re = Str.regexp {|^\([a-zA-Z]\)[:|]\([/\].*\)$|}  (* e.g. c:\ or z|/ *)
...
let parse (uri: string) : string =
  ...
  let path = unescape path in (* this uses regexp internally *)
  ...
  if Str.string_match dos_url_re path 0 then begin
    let drive_letter = Str.matched_group 1 path in
    let rest = Str.matched_group 2 path in
    drive_letter ^ ":" ^ rest
  end else if String.length path > 0 && path.[0] = '/' then
...
let create (path: string) : string =
  let absolute_path = if Str.string_match dos_re path 0 then
    let drive_letter = Str.matched_group 1 path in
    let rest = Str.matched_group 2 path in
    Printf.sprintf "%s:%s" drive_letter (Str.global_replace backslash_re "/" rest)
